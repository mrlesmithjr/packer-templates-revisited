...
variable "iso_checksum" {
  type    = string
  default = "02257c3ec27e45d9f022c181a69b59da67e5c72871cdb4f9a69db323a1fad58093f2e69702d29aa98f5f65e920e0b970d816475a5a936e1f3bf33832257b7e92"
}
...
variable "vagrant_box_tag" {
  type    = string
  default = "${env("VAGRANT_CLOUD_USER")}/bullseye64"
}
...
source "proxmox-iso" "autogenerated_1" {
  boot_command            = ["<esc><wait>", "install <wait>", " auto=true", " priority=critical", " url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian/preseed.cfg <wait>", "<enter>"]
...
source "qemu" "autogenerated_2" {
  ...
  iso_url          = "${var.iso_url}"
  memory           = "${var.memory}"
  output_directory = "output-${var.vm_name}-${build.type}-${local.timestamp}"
...
source "vagrant" "autogenerated_3" {
  communicator = "${var.communicator}"
  output_dir   = "output-${var.vm_name}-${build.type}-${local.timestamp}"
  provider     = "virtualbox"
...
source "virtualbox-ovf" "autogenerated_5" {
  boot_wait        = "${var.boot_wait}"
  communicator     = "${var.communicator}"
  headless         = "${var.headless}"
  output_directory = "output-${var.vm_name}-${build.type}-${local.timestamp}"
...
source "vmware-iso" "autogenerated_6" {
  boot_command      = ["<esc><wait>", "install <wait>", " auto=true", " priority=critical", " url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian/preseed.cfg <wait>", "<enter>"]
  boot_wait         = "${var.boot_wait}"
  communicator      = "${var.communicator}"
  cpus              = "${var.cpus}"
...
source "vmware-vmx" "autogenerated_7" {
  boot_wait        = "${var.boot_wait}"
  communicator     = "${var.communicator}"
  headless         = "${var.headless}"
  output_directory = "output-${var.vm_name}-${build.type}-${local.timestamp}"
...
source "vsphere-iso" "autogenerated_8" {
  CPUS                 = "${var.cpus}"
  RAM                  = "${var.memory}"
  boot_command         = ["<esc><wait>", "install <wait>", " auto=true", " priority=critical", " url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian/preseed.cfg <wait>", "<enter>"]
  boot_wait            = "${var.boot_wait}"
...
build {
  sources = ["source.proxmox-iso.autogenerated_1", "source.qemu.autogenerated_2", "source.vagrant.autogenerated_3", "source.virtualbox-iso.autogenerated_4", "source.virtualbox-ovf.autogenerated_5", "source.vmware-iso.autogenerated_6", "source.vmware-vmx.autogenerated_7", "source.vsphere-iso.autogenerated_8"]
  ...
  post-processors {
    ...
    post-processor "shell-local" {
      inline  = ["ln -nfs ${path.root}/output-${var.vm_name}-${build.type}-${local.timestamp} ${path.root}/output-${var.vm_name}-${build.type}-latest"]
