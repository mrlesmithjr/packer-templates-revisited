...
cd "$PROJ_ROOT/Rocky"
packer validate -syntax-only rocky8.json
...
cd "$PROJ_ROOT/Debian"
packer build -only virtualbox-iso debian-buster64.json
...
packer build -only virtualbox-iso debian-stretch64.json
...
# packer build -only proxmox -only virtualbox-iso -only vmware-iso rocky8.json
# packer build -only qemu rocky8.json
...
packer build -only virtualbox-iso ubuntu-xenial64.json
...
packer build -only virtualbox-iso ubuntu-focal64.json
...
cd "$PROJ_ROOT/CentOS"
packer build -only virtualbox-iso centos7-desktop.json
# packer build -only virtualbox-iso -only vmware-iso centos7-desktop.json

cd "$PROJ_ROOT/Ubuntu"
packer build -only virtualbox-iso buntu-bionic64-desktop.json
# packer build -only virtualbox-iso -only vmware-iso ubuntu-bionic64-desktop.json
packer build -only virtualbox-iso ubuntu-focal64-desktop.json
