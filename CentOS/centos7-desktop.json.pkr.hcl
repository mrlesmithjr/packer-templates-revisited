...
variable "vagrant_box_tag" {
  type    = string
  default = "${env("VAGRANT_CLOUD_USER")}/centos7-desktop"
}
...
source "proxmox-iso" "centos" {
  boot_command            = ["<tab> inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos/ks.7.cfg<enter><wait>"]
...
build {
  sources = ["source.proxmox-iso.centos", "source.qemu.centos", "source.vagrant.centos", "source.virtualbox-iso.centos", "source.virtualbox-ovf.centos", "source.vmware-iso.centos", "source.vmware-vmx.centos", "source.vsphere-iso.centos"]
  ...
  post-processors {
    post-processor "vagrant" {
      ...
      only                = ["qemu.centos", "virtualbox-iso.centos", "vmware-iso.centos"]
    ...
    post-processor "shell-local" {
      ...
      only    = ["virtualbox-iso.centos", "vmware-iso.centos"]
