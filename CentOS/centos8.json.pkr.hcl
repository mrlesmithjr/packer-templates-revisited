...
variable "vagrant_box_tag" {
  type    = string
  default = "${env("VAGRANT_CLOUD_USER")}/centos8"
}
...
source "proxmox-iso" "centos" {
  boot_command            = ["<tab> inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos/ks.8.cfg<enter><wait>"]
...
build {
  ...
  post-processors {
    post-processor "vagrant" {
      ...
      only                = ["qemu.centos", "virtualbox-iso.centos", "virtualbox-ovf.centos", "vmware-iso.centos"]
